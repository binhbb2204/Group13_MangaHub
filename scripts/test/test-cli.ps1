...
# Resolve repository root and bin path regardless of current working directory
$repoRoot = (Get-Item $PSScriptRoot).Parent.Parent.FullName
$binPath = Join-Path $repoRoot "bin"
$exe = Join-Path $binPath "mangahub.exe"
...
& $exe auth --help | Select-Object -First 1
if ($LASTEXITCODE -eq 0) {
...
if (Test-Path $configPath) {
    ...
    Write-Host "⚠ Configuration not initialized. Run: $exe init" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "  1. Initialize (if not done): $exe init" -ForegroundColor White
Write-Host "  2. Start server: go run cmd/api-server/main.go" -ForegroundColor White
Write-Host "  3. Test registration: $exe auth register --username testuser --email test@example.com" -ForegroundColor White